...
```ts
// Direct validation
const date = p.date('2024-01-15')

// Parse unknown input
const date = p.date.parse(jsonData)

// Check validity without throwing
if (p.date.isValid('2024-02-29')) {
...
const date = p.date.parse(userInput)

// Safe parsing (returns result object)
const result = p.date.safeParse(userInput)
if (result.success) {
...
const filled = form.fill({
  fields: {
    checkIn: p.date('2024-06-01'),
    checkOut: p.date('2024-06-07')
  }
})
```
...
p.date('2023-02-29')  // Not a leap year
...
- [Date Field](/sdk/artifacts/form/field#date--datetime--time) - Date field type
