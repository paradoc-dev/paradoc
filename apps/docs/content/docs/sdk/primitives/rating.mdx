...
const rating = p.rating.parse(input, { min: 1, max: 5, step: 0.5 })

// Check validity without throwing
if (p.rating.isValid(4)) {
...
const rating = p.rating.parse(userInput)

// Safe parsing (returns result object)
const result = p.rating.safeParse(userInput)
if (result.success) {
...
const filled = form.fill({
  fields: {
    overallRating: p.rating(4),
    serviceRating: p.rating.parse(8, { min: 1, max: 10 })
  }
})
```
...
<MethodTable methods={[
  ...
  { name: 'isValid', params: 'input: number, options?: RatingOptions', returns: 'boolean', description: 'Check if a number is a valid rating' },
]} />
...
p.rating.parse(4, { step: 1 })      // Valid: 4
p.rating.parse(4.5, { step: 1 })    // Throws: not aligned

// Step of 0.5 (half stars)
p.rating.parse(4, { step: 0.5 })    // Valid: 4
p.rating.parse(4.5, { step: 0.5 })  // Valid: 4.5
p.rating.parse(4.3, { step: 0.5 })  // Throws: not aligned

// Step of 0.1 (tenths)
p.rating.parse(4.7, { step: 0.1 })  // Valid: 4.7
```
...
// With custom options
p.rating.parse(4.5, { min: 1, max: 5, step: 0.5 }) // Valid: half-stars enabled
p.rating.parse(0, { min: 0, max: 5 })              // Valid: custom min
