...
The Duration primitive uses ISO 8601 duration format: Weeks use `W`, seconds may have a fractional part, and `P` or `PT` alone is invalid.

```
P[n]Y[n]M[n]W[n]DT[n]H[n]M[n(.n)]S
```
...
- [Duration Field](/sdk/artifacts/form/field#temporal) - Duration field type
