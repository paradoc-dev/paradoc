...
if (sum.success) valueToString(sum.value) // "30"
```

If you already have an AST, `evaluate(ast, ctx)` runs it directly.

`evaluateExpression` is the safe source API. Low-level `evaluate`, `parseOrThrow`, and direct `Decimal` operations may throw. Internal numbers remain decimals; converting them to JavaScript numbers is explicitly lossy. Division retains up to 20 fractional places. The engine bounds expression nesting, regex work, and decimal scale.

`coalesce` stops at the first non-null argument. Dates use canonical `YYYY-MM-DD` values, datetimes representing instants require `Z` or an explicit offset, and calendar month/year addition clamps to the last valid day. Diagnostic offsets count UTF-16 code units.
...
`sum`, `count`, `min`, `max`, `avg`, `any`, and `all` read their first argument as a path into a list rather than as a value; the [Logic](/concepts/logic#aggregating-lists) page covers the language rules. Two things are the host's to supply.

**Row visibility.** Pass `rowVisible` to `createContext` to keep hidden rows out. It receives the list's path and the row's position at every list level, and every row counts when it is absent. `@paradoc/core` supplies it from the artifact's `visible` conditions.

```ts
const ctx = createContext(
  { fields: { items: [{ qty: 2 }, { qty: 5 }] } },
  { rowVisible: (listPath, indices) => !(listPath === 'fields.items' && indices[0] === 1) },
)
evaluateExpression('sum(fields.items.qty)', ctx) // 2: the second row is hidden
```

**Row types.** A `TypeEnv` types a list as `T.array(...)` and registers each item member under the list's own path, typed per row. The checker then knows that `fields.items.qty` is a number read from every row: accepted inside an aggregate, reported with the `invalid-aggregate` code anywhere else.
...
check('sum(fields.items.qty)', env).type    // { kind: 'number' }
check('fields.items.qty > 1', env).diagnostics[0].code // 'invalid-aggregate'
```

A money sum over rows in more than one currency fails with the `currency-mismatch` evaluation code.
