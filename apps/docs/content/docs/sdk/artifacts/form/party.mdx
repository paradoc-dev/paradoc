...
An empty list for a `max > 1` role counts as an absent role. A role whose `required` is `false`, or whose `required` expression evaluates to false, may be absent or empty; `min` then applies only to a list that names parties.
...
<PropertiesTable properties={[
  ...
  { name: 'payment', type: 'FormPayment', description: 'Payment requirement', isOptional: true },
]} />
...
<PropertiesTable properties={[
  ...
  { name: 'witnesses', type: 'number', description: 'Number of caller-declared witness attestations required for this signature', isOptional: true },
  { name: 'notarized', type: 'boolean', description: 'Whether an attesting witness must be declared with notary status', isOptional: true },
]} />

#### FormPayment Properties

<PropertiesTable properties={[
  { name: 'required', type: 'boolean', description: 'Whether payment is required for this role', isOptional: true },
  { name: 'amount', type: 'Money | MoneyExpression', description: 'Amount owed: a fixed Money value, or a MoneyExpression resolved from filled data at request time' },
]} />
...
<MethodChain
  ...
  methods={[
    ...
    { name: 'payment', params: 'value: FormPayment', description: 'Set the payment requirement' },
...
const form = p.form()
  ...
  .field('loanAmount', field.number().label('Loan amount').build())
  .def('needsGuarantor', 'fields.loanAmount > 100000')
...
`RuntimeParty` extends `Party` with the `id` a filled form gives each party. Signatories, captures, and attestations reference a party by it:

```typescript
type RuntimeParty = (Person | Organization) & {
  id: string  // `<roleId>-<index>`, assigned by core
}
```

The id is always `{roleId}-{index}`, the party's position in its role. `fill`, `update`, `setParty`, `addParty`, and `runtimeFormFromJSON` assign it, so you can leave it out. An id you do supply must match that position, or the call throws. `removeParty` gives each later party the id of its new position and moves its signatories with it.

A party record holds only its `Person` or `Organization` members. A member the schema does not declare, such as `address` or `phone`, is refused. Declare those as fields beside the party instead; the [`Party`](/components/party) component prints them by path.
...
const filled = form.fill({
  ...
  parties: {
    buyer: {
      id: 'buyer-0',  // Optional: core assigns it
      name: 'John Smith',
      firstName: 'John',
      lastName: 'Smith'
    },
    seller: {
      id: 'seller-0',  // Optional: core assigns it
...
const filled = form.fill({
  ...
  parties: {
    buyer: {
      id: 'buyer-0',
      ...p.person().name('John Smith').build()
    },
    seller: {
      id: 'seller-0',
      ...p.organization()
