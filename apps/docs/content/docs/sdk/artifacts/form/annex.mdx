...
const form = p.form()
  ...
  .field('amount', field.number().label('Loan amount').build())
  .field('age', field.number().label('Applicant age').build())
  .def('isLargeAmount', 'fields.amount > 50000')
  .def('isMinor', 'fields.age < 18')
...
An additional annex value must still be an [Attachment](/schemas/primitives/attachment). Without `allowAdditionalAnnexes: true`, a payload with an undeclared annex key fails validation. The inferred payload type (`InferFormPayload`) follows the same rule for the object pattern and the builder.
