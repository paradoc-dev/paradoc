...
<PropertiesTable properties={[
  { name: 'contents', type: 'Record<string, BundleAssemblyEntry>', description: 'One entry per bundle content key, as for assemble()' },
...
The result, a `SealedBundle`, carries `pdf`, `canonicalPdfHash`, `packetHash` (a digest over the merged PDF's hash and every part's), `signers`, `signatureMap`, and `parts`. Each part reports its `kind` (`'sealed'`, `'rendered'`, or `'annex'`), its page range in the packet, and its own digest.

Part signer ids are scoped as `<part>/<signerId>`, because two parts that both say `signer-1` are not evidence that one person signs both. Map them with `signers` when they are. A failure throws `BundleSealError`, whose `problems` lists every issue and `part` names the content key when there is one.
