...
<PropertiesTable properties={[
  { name: 'contents', type: 'Record<string, AssemblyContentEntry>', description: 'Content to assemble, keyed by bundle content key' },
...
An `AssemblyContentEntry` is either a filled artifact instance or bytes
supplied directly:
...
<PropertiesTable properties={[
  { name: 'contents', type: 'Record<string, AssemblyContentEntry>', description: 'One entry per bundle content key, as for assemble()' },
