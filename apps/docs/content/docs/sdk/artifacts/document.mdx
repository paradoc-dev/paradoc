...
`render()` takes `ArtifactLayerRenderOptions`. A text, PDF, or DOCX layer renders through the built-in engines; a layer whose MIME type has an entry in `renderers` renders through that entry instead.

<PropertiesTable properties={[
  { name: 'layer', type: 'string', description: 'Layer key to render. Defaults to the target layer, then defaultLayer', isOptional: true },
  { name: 'renderers', type: 'RendererRegistry', description: 'Renderers keyed by layer MIME type. This is how a React layer (`text/tsx`, `text/jsx`) renders', isOptional: true },
