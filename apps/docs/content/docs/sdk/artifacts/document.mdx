...
```ts
para.document(input: DocumentInput, options?: ArtifactInstanceOptions): DocumentInstance
```
...
<PropertiesTable properties={[
  ...
  { name: 'render', type: '(options?: ArtifactLayerRenderOptions) => Promise<string | Uint8Array>', description: 'Render document content', isMethod: true },
]} />
...
// Render
const output = await final.render({ renderer: renderLayer() })
```
