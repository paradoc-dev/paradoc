...
const doc = p.document.from(jsonData)

// Safe parsing (returns result object)
const result = p.document.safeFrom(jsonData)
if (result.success) {
...
```ts
p.document(input: DocumentInput, options?: ArtifactInstanceOptions): DocumentInstance
```

`options.resolver` reads the bytes of any file-backed layer this document
declares. It's bound once here, at construction, and every instance derived
from the document — every mutator and `clone` — carries it; `render()` itself
takes no resolver. See [ArtifactInstanceOptions](/sdk/resolvers#artifactinstanceoptions).
...
<PropertiesTable properties={[
  ...
  { name: 'prepare', type: '(targetLayer?: string) => DraftDocument', description: 'Create a draft for a layer (defaults to defaultLayer)', isMethod: true },
  { name: 'render', type: '(options?: ArtifactLayerRenderOptions) => Promise<string | Uint8Array>', description: 'Render document content. See [Render options](#render-options)', isMethod: true },
]} />
...
Parse documents from unknown data sources. Both accept an
[`ArtifactInstanceOptions`](/sdk/resolvers#artifactinstanceoptions) to bind a
resolver for any file-backed layer:

<MethodTable methods={[
  { name: 'from', params: 'input: unknown, options?: ArtifactInstanceOptions', returns: 'DocumentInstance', description: 'Parse unknown input (throws on error)' },
  { name: 'safeFrom', params: 'input: unknown, options?: ArtifactInstanceOptions', returns: 'Result<DocumentInstance>', description: 'Parse unknown input (returns result object)' },
]} />
...
// Render
const output = await final.render()
```

### DraftDocument

Created by `doc.prepare()`.
...
<PropertiesTable properties={[
  ...
  { name: 'setTargetLayer', type: '(layer: string) => DraftDocument', description: 'Change the target layer', isMethod: true },
  { name: 'finalize', type: '() => FinalDocument', description: 'Transition to final phase', isMethod: true },
  { name: 'render', type: '(options?: ArtifactLayerRenderOptions) => Promise<string | Uint8Array>', description: 'Render the document', isMethod: true },
  ...
  { name: 'clone', type: '() => DraftDocument', description: 'Create an exact copy', isMethod: true },
]} />
...
<PropertiesTable properties={[
  ...
  { name: 'toYAML', type: '() => string', description: 'Serialize to YAML', isMethod: true },
  { name: 'clone', type: '() => FinalDocument', description: 'Create an exact copy', isMethod: true },
]} />

## Render options

`render()` takes `ArtifactLayerRenderOptions`. A renderer registered for the layer's MIME type runs first. Otherwise a document layer returns its resolved content unchanged.

<PropertiesTable properties={[
  { name: 'layer', type: 'string', description: 'Layer key to render. Defaults to the target layer, then defaultLayer', isOptional: true },
  { name: 'renderers', type: 'RendererRegistry', description: 'Renderers keyed by layer MIME type', isOptional: true },
