...
| `images` | `PdfImage[]` | Bytes for every image `src` that is neither a `data:` URI nor inline SVG markup. No engine fetches images; a missing one fails the render |
...
| `missingImages` | Image `src` values that are neither `data:` URIs nor inline SVG markup, which a render needs bytes for |
