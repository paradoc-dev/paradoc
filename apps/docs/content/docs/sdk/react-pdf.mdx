...
| `fonts` | `PdfFontResource[]` | Application font faces (Chromium adapter only) |
...
The preview's [`Pages`](/components/pages) component reports its plan through `onPaginate`. Send that plan with the render, together with the same furniture, and the PDF matches the preview page for page.
...
| --- | --- | --- | --- |
| `takumi` (default) | `ltr` | header, footer, stamp | WebAssembly. Draws from a verified vocabulary of Tailwind classes |
...
A right-to-left document on Takumi throws `UnsupportedDirectionError`. Application `fonts` or `applicationCss` on Takumi throw `UnsupportedApplicationTypographyError`. Classes outside Takumi's vocabulary, or images with no bytes, throw `UnsupportedPdfContentError`, naming each one.
