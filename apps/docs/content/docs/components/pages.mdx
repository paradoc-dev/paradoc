...
on. The PDF render from [`@paradoc/react-pdf`](/sdk/react-pdf) takes the same
object.
...
stamp that lays out taller than the paper, or with a word wider than it, fails
by name rather than being cut off at the paper's edge on every page. It is
...
Each engine declares which slots it draws, and both engines draw all three.
The experimental Chromium adapter prints the header and the footer through its
own print templates and repeats the stamp on every page; it cannot print a page
number in the stamp, and refuses one by name. An engine that does not draw a
slot fails naming the engine and the slot rather than writing a document whose
every page is missing its header.
...
<ComponentVariant name="pages" variant="draft-watermark" />
