---
title: render
description: Render a Paradoc artifact from a verified registry as text, PDF, or DOCX
ogTitle: render — Paradoc MCP
ogDescription: Render a Paradoc artifact from a verified registry using the MCP server
---

Renders a Paradoc form artifact from a verified registry. Looks up the artifact by `registry_id` and `artifact_name`, fetches it from the registry, validates, fills with data, and renders via the appropriate renderer (text, PDF, DOCX). Returns a short download link by default, or content inline.
...
| `data` | `object` | Yes | Data to fill before rendering: `{ fields: { fieldId: value }, parties: { roleId: { id, name, ... } }, annexes: { annexId: attachment } }`. |
