...
const moveInChecklist = p.checklist({
  ...
  defaultLayer: 'text',
  layers: { text: { kind: 'inline', mimeType: 'text/plain', text: 'Move-In Checklist' } },
})
```
...
const moveInChecklist = p
  ...
  .inlineLayer('text', { mimeType: 'text/plain', text: 'Move-In Checklist' })
  .defaultLayer('text')
  .build()
```
</Step>
...
Bundle assembly selects the built-in renderer for each layer MIME type. Pass a
`renderers` map only when a MIME type needs a custom override.
...
const bundle = p
  ...
  .registry('shared', '@acme/forms/disclosure') // Registry reference
  .build()
```
