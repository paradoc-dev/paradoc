---
title: Forms
description: Create, fill, and render a simple lease agreement
ogTitle: Create a form with Paradoc
ogDescription: Build a lease agreement with fields, parties, templates, and rendering
---

import { Step, Steps } from 'fumadocs-ui/components/steps';

This guide walks through creating a lease agreement, filling it with data, and rendering the output.

<Steps>

<Step>
## Create a new project

Create a new project, set up the directory structure, and download the W-9 form:

<Tabs items={['npm', 'pnpm', 'yarn', 'bun']} groupId="package-manager" persist>
  <Tab value="npm">
    ```bash
    mkdir my-form && cd my-form
    npm init -y
    ```
  </Tab>
  <Tab value="pnpm">
    ```bash
    mkdir my-form && cd my-form
    pnpm init -y
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    mkdir my-form && cd my-form
    yarn init -y
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    mkdir my-form && cd my-form
    bun init -y
    ```
  </Tab>
</Tabs>
</Step>


<Step>
## Install the SDK

Install the SDK and unified renderer.

<Tabs items={['npm', 'pnpm', 'yarn', 'bun']} groupId="package-manager" persist>
  <Tab value="npm">
    ```bash
    npm install @paradoc/sdk
    ```
  </Tab>
  <Tab value="pnpm">
    ```bash
    pnpm add @paradoc/sdk
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn add @paradoc/sdk
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bun add @paradoc/sdk
    ```
  </Tab>
</Tabs>
</Step>

<Step>
## Define the Form

A form needs a name, fields, and parties. Here's a simple lease agreement:

```ts tab="Object Pattern"
import { p } from '@paradoc/sdk'

const lease = p.form({
  name: 'lease-agreement',
  version: '1.0.0',
  title: 'Residential Lease Agreement',
  parties: {
    landlord: { label: 'Landlord', partyType: 'person', signature: { required: true } },
    tenant: { label: 'Tenant', partyType: 'person', signature: { required: true } },
  },
  fields: {
    address: { type: 'address', label: 'Property Address', required: true },
    monthlyRent: { type: 'money', label: 'Monthly Rent', required: true },
    startDate: { type: 'date', label: 'Start Date', required: true },
    endDate: { type: 'date', label: 'End Date', required: true },
  },
})
```

```ts tab="Builder Pattern"
import { p } from '@paradoc/sdk'

const lease = p
  .form()
  .name('lease-agreement')
  .version('1.0.0')
  .title('Residential Lease Agreement')
  .parties({
    landlord: p.party().label('Landlord').partyType('person').signature({ required: true }),
    tenant: p.party().label('Tenant').partyType('person').signature({ required: true }),
  })
  .fields({
    address: p.field.address().label('Property Address').required(),
    monthlyRent: p.field.money().label('Monthly Rent').required(),
    startDate: p.field.date().label('Start Date').required(),
    endDate: p.field.date().label('End Date').required(),
  })
  .build()
```
</Step>

<Step>
## Add a Layer

Templates define how forms render. Add an inline Markdown template using
Paradoc's deterministic template syntax:

```ts tab="Object Pattern"
const lease = p.form({
  name: 'lease-agreement',
  // ...fields and parties
  defaultLayer: 'markdown',
  layers: {
    markdown: {
      kind: 'inline',
      mimeType: 'text/markdown',
      text: `
# Residential Lease Agreement

**Property:** {{fields.address}}
**Monthly Rent:** {{fields.monthlyRent}}
**Term:** {{fields.startDate}} to {{fields.endDate}}

**Landlord:** {{parties.landlord.name}}
**Tenant:** {{parties.tenant.name}}
      `,
    },
  },
})
```

```ts tab="Builder Pattern"
const lease = p
  .form()
  .name('lease-agreement')
  // ...fields and parties
  .defaultLayer('markdown')
  .layers({
    markdown: p.layer.inline().mimeType('text/markdown').text(`
# Residential Lease Agreement

**Property:** {{fields.address}}
**Monthly Rent:** {{fields.monthlyRent}}
**Term:** {{fields.startDate}} to {{fields.endDate}}

**Landlord:** {{parties.landlord.name}}
**Tenant:** {{parties.tenant.name}}
    `),
  })
  .build()
```
</Step>

<Step>
## Fill the Form

Use `.fill()` to create a runtime instance with data:

```ts
const filled = lease.fill({
  parties: {
    landlord: { id: 'landlord-1', name: 'Jane Smith' },
    tenant: { id: 'tenant-1', name: 'John Doe' },
  },
  fields: {
    address: {
      line1: '123 Main St',
      locality: 'Portland',
      region: 'OR',
      postalCode: '97201',
      country: 'USA',
    },
    monthlyRent: { amount: 1500, currency: 'USD' },
    startDate: '2025-02-01',
    endDate: '2026-01-31',
  },
})
```

Party data requires an `id` field—this is used for signature tracking.
</Step>

<Step>
## Render the Form

Render the filled form. Paradoc selects the built-in implementation from the
layer's MIME type:

```ts
const output = await filled.render()

console.log(output)
```

Output:

```markdown
# Residential Lease Agreement

**Property:** 123 Main St, Portland, OR 97201, USA
**Monthly Rent:** $1,500.00
**Term:** Feb 1, 2025 to Jan 31, 2026

**Landlord:** Jane Smith
**Tenant:** John Doe
```

Fields are automatically serialized—`money` becomes `$1,500.00`, `address` becomes a formatted string.
</Step>

<Step>
## Save the Artifact

Export the form to JSON or YAML for storage or version control:

```ts
const json = lease.toJSON() // or lease.toYAML()
console.log(JSON.stringify(json, null, 2))
```

```json tab="JSON"
{
  "$schema": "https://schema.paradoc.dev/__SCHEMA_VERSION__.json",
  "kind": "form",
  "name": "lease-agreement",
  "version": "1.0.0",
  "title": "Residential Lease Agreement",
  "fields": {
    "address": { "type": "address", "label": "Property Address", "required": true },
    "monthlyRent": { "type": "money", "label": "Monthly Rent", "required": true },
    "startDate": { "type": "date", "label": "Start Date", "required": true },
    "endDate": { "type": "date", "label": "End Date", "required": true }
  },
  "parties": {
    "landlord": { "label": "Landlord", "signature": { "required": true } },
    "tenant": { "label": "Tenant", "signature": { "required": true } }
  }
}
```

```yaml tab="YAML"
# yaml-language-server: $schema=https://schema.paradoc.dev/__SCHEMA_VERSION__.json
$schema: https://schema.paradoc.dev/__SCHEMA_VERSION__.json
kind: form
name: lease-agreement
version: 1.0.0
title: Residential Lease Agreement
fields:
  address:
    type: address
    label: Property Address
    required: true
  monthlyRent:
    type: money
    label: Monthly Rent
    required: true
  startDate:
    type: date
    label: Start Date
    required: true
  endDate:
    type: date
    label: End Date
    required: true
parties:
  landlord:
    label: Landlord
    partyType: person
    signature:
      required: true
  tenant:
    label: Tenant
    partyType: person
    signature:
      required: true
```

Load it back with `p.load()`:

```ts
const loaded = p.load(jsonString) // works with JSON or YAML
```

`load()` requires the `$schema` that `toJSON()` and `toYAML()` write. A file written for an earlier [schema version](/schemas#loading-rules) fails to load until [`paradoc migrate`](/cli/commands/migrate) upgrades it.
</Step>
</Steps>
