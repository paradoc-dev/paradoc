...
Layers in older schema versions could declare slots in `signatureBlocks` and
`anchorBlocks`. Those fields are removed (see the
[version history](/schemas#version-history)). Run
