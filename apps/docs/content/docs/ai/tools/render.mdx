...
`presentation` controls the application-facing result. The separate `maxOutputBytes` [config](/ai#configuration) value controls the copy sent to the model. File-backed layers load relative to the source's base URL; with an [artifact source](/ai/tools#sources), set `base_url`.
