...
| `candidates` | `FillTarget[]` | Targets that can be filled now, in declaration order |
...
A `FillTarget` has `kind` (`field`, `party`, or `annex`), `key`, `required`, and `order`. A `FillItemState` adds `visible`, `status` (`hidden`, `optional`, or `required`), `filled`, and `blockedBy` (the unfilled targets that keep it hidden). These nested entries keep their camelCase keys.
