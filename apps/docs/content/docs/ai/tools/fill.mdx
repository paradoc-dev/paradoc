...
Creates a progressive form or checklist draft, applies defaults, and validates supplied values. The response reports `accepted` for the patch and `complete` for full readiness. Drafts may render before completion.
