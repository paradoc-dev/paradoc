...
1. **Create the registry** — [`paradoc registry make`](/cli/commands/registry/make) generates a `registry.json` with your namespace and metadata.
...
```bash
paradoc registry make --name @acme --title "Acme Forms Registry"
paradoc registry catalog add residential-lease ./artifacts/residential-lease/artifact.yaml
