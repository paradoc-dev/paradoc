...
```bash
paradoc attach <artifact> <file> [options]
```
...
```bash
paradoc attach my-form.yaml ./lease-template.pdf
```

Attach a layer with explicit options:

```bash
paradoc attach my-form.yaml ./lease-template.pdf --name pdf --title "Lease Template" -y
```

Attach a file as instructions:

```bash
paradoc attach my-form.yaml ./instructions.md --as instructions
```

Attach a file as agent instructions:

```bash
paradoc attach my-form.yaml ./agent-guide.md --as agent-instructions
```

Preview changes without writing:

```bash
paradoc attach my-form.yaml ./lease-template.pdf --dry-run
```
...
The attached file must be in the artifact's directory; copy it there first. Layer names use camelCase and default to the camelCased filename (`lease-template.pdf` becomes `leaseTemplate`). MIME detection includes PDF, DOCX, TSX, JSX, common text and image formats, and other known extensions; an unknown extension uses `application/octet-stream` with a warning.
