...
```bash
paradoc inspect form-template.pdf
```

Filter fields by pattern:

```bash
paradoc inspect form-template.pdf --filter "address_*"
```

Output as JSON:

```bash
paradoc inspect form-template.pdf --format json
```

Get summary only:

```bash
paradoc inspect form-template.pdf --summary
```

Save to file:

```bash
paradoc inspect form-template.pdf --out fields.json
```
...
```
Fields from /Users/you/my-project/form-template.pdf:
┌──────────────────┬──────────┬──────────┬─────────┐
...
```json
{
  "source": "/Users/you/my-project/form-template.pdf",
  "total": 15,
  "breakdown": {
    "text": 10,
    "checkbox": 3,
