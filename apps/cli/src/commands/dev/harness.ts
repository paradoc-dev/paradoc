/**
 * The preview page `paradoc dev` serves, as source for Vite to compile.
 *
 * These modules are generated rather than shipped as files because they are the
 * project's code, not `paradoc`'s: Vite compiles them with the project's React,
 ...
 * frame `paradoc dev` draws never depends on the project's Tailwind version, and a
...
/** The preview's own frame. Plain CSS: it is `paradoc dev`'s, not the project's. */
const CHROME = `html,
body,
#root {
  height: 100%;
...
export function manifestModule(entries: readonly DiscoveredComposition[]): string {
	...
	return `// Generated by \`paradoc dev\`. Nothing reads this file from disk.
...
const CLIENT = `/* Generated by \`paradoc dev\`. */
  ...
  if (entries.length === 0) {
    return (
      <Failure
        ...
        messages={[
          "paradoc dev previews every .tsx or .jsx file under a compositions/ directory. " +
