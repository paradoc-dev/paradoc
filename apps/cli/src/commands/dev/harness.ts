 ...
 * is what a composition and `@paradoc/react` are written in, and it is compiled
 ...
 * package. `@paradoc/react` is named twice because a published install has its
 * components in `dist` and a linked workspace one has them in `src`; the two
 * are alternatives and a missing one costs nothing. Without them the components'
 * own classes are never generated, the document lays out shorter than it should,
 * and the page plan the PDF is hinted with is measured against the wrong
 * document.
 */
export function stylesheetModule(
	...
	return `@import "@paradoc/react/styles.css";
@import "tailwindcss";

@source ${to(`${root}/**/*.{tsx,jsx}`)};
${excluded}
@source ${to(`${packageDir}/dist/**/*.js`)};
@source ${to(`${packageDir}/src/**/*.{ts,tsx}`)};
...
import { Pages } from "@paradoc/react";
