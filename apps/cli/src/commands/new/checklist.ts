...
/**
 * Implementation function to create a new checklist artifact
 * @param name - Name of the checklist
 * @param options - Checklist creation options
 */
async function createChecklistImpl(
	name: string,
	...
	const defaultTitle = name
		...
		.join(" ");
	const defaultSlug = generateSlug(name, "checklist");

	try {
		if (!yes) {
			// Interactive mode
			console.log();
			console.log(kleur.bold("Create Checklist"));
			console.log();
			...
			const { dir: inputDir } = await prompts({
				type: "text",
				name: "dir",
				message: `Where would you like to save this checklist? ${kleur.gray(`(${artifactSlug} or . for current)`)}:`,
				initial: ".",
	...
	// Generate template
	const template = generateChecklistTemplate(artifactSlug, artifactTitle, {
...
/**
 * Create the 'checklist' subcommand
 */
export function createChecklistCommand(): Command {
	...
	checklist
		...
		.option("--item <text>", "Add item (repeatable)", collect, [])
		.option("--dry-run", "Preview without creating files")
		.option("--format <format>", "Output format (json|yaml)", "json")
		.action(async (name: string, options: ChecklistOptions) => {
			await createChecklistImpl(name, options);
		});
