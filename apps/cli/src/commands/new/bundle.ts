...
import { generateBundleTemplate } from "../../utils/templates.js";
...
/**
 * Implementation function to create a new bundle artifact
 * @param name - Name of the bundle
 * @param options - Bundle creation options
 */
async function createBundleImpl(
	name: string,
	...
	const defaultTitle = name
		...
		.join(" ");
	const defaultSlug = generateSlug(name, "bundle");

	try {
		if (!yes) {
			// Interactive mode
			console.log();
			console.log(kleur.bold("Create Bundle"));
			console.log();
			...
			const { dir: inputDir } = await prompts({
				type: "text",
				name: "dir",
				message: `Where would you like to save this bundle? ${kleur.gray(`(${artifactSlug} or . for current)`)}:`,
				initial: ".",
	...
	// Generate template
	const template = generateBundleTemplate(artifactSlug, artifactTitle, {
...
export function createBundleCommand(): Command {
	...
	bundle
		...
		.action(async (name: string, options: BundleOptions) => {
			await createBundleImpl(name, options);
		});
