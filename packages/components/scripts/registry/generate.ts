 ...
 * ${describe}
 *
 * Generated by \`pnpm registry:build\` in the private \`@paradoc/components\`
 * workspace from the file beside it. Do not edit: the registry's freshness
 * test regenerates this module and fails on a difference.
 */
  ...
  const index: RegistryIndex = {
    ...
    items: items.map((item, position) => ({
      ...
      files: item.files.map((file) => ({
        path: `paradoc/packages/components/src/${file.path}`,
        type: file.type,
        target: file.target,
      })),
    })),
