...
type CoreRuleResult = { ruleId: string; message?: string }
type CoreRules = { valid: boolean; errors: CoreRuleResult[]; warnings: CoreRuleResult[] }

function ruleViolation(result: CoreRuleResult, fallback: string): RuleViolation {
...
export async function executeGetFillState(
	...
	try {
		...
		if (isForm(artifact)) {
			...
			const runtime = draft.runtimeState
			...
			if (!runtime.resolved) {
				const issues: ToolError[] = runtime.issues.map((issue) => ({ code: 'logic_unresolved', message: issue.message, ...(issue.path.length > 0 ? { path: [...issue.path] } : {}) }))
				return stateOutput('form', state, { valid: false, errors: [], warnings: [] }, draft.context, issues)
			}
			return stateOutput('form', state, draft.validateRules(), draft.context)
		}
