...
import { UpdateFillInputSchema, type UpdateFillInput, type UpdateFillOutput } from '../contracts'
import { artifactKind, asChecklistPayload, asFormPayload, contextOptions, contextSnapshot, formDraftPayload, makeResolver } from '../artifact'
...
export async function executeUpdateFill(
	...
	try {
		const normalized = UpdateFillInputSchema.parse(input)
		...
		if (isForm(artifact)) {
			...
			if (!initial.success) return { accepted: false, complete: false, artifact_kind: 'form', errors: validationErrors(initial.error), error: errorFromUnknown(initial.error, 'validation_error') }
			...
			if (normalized.patch !== undefined) {
				...
				if (!updated.success) return { accepted: false, complete: false, artifact_kind: 'form', errors: validationErrors(updated.error), error: errorFromUnknown(updated.error, 'validation_error') }
				draft = updated.data
			}
		...
		if (isChecklist(artifact)) {
			...
			if (!initial.success) return { accepted: false, complete: false, artifact_kind: 'checklist', errors: validationErrors(initial.error), error: errorFromUnknown(initial.error, 'validation_error') }
			let draft = initial.data
			if (normalized.patch !== undefined) {
				const updated = draft.safeUpdate(asChecklistPayload(normalized.patch))
				if (!updated.success) return { accepted: false, complete: false, artifact_kind: 'checklist', errors: validationErrors(updated.error), error: errorFromUnknown(updated.error, 'validation_error') }
				draft = updated.data
			}
			for (const path of normalized.clear ?? []) draft = draft.clear(path as never)
			for (const path of normalized.reset ?? []) draft = draft.reset(path as never)
		...
		return { accepted: false, complete: false, ...(kind ? { artifact_kind: kind } : {}), error: { code: 'unsupported_artifact', message: 'Only form and checklist artifacts support updates.' } }
	} catch (error) {
