...
import { inspectAcroFormFields as inspectExisting, pdfRenderer as existingPdfRenderer } from '@paradoc/renderer-pdf'
...
describe('PDF renderer parity', () => {
  ...
  it('fills the real PDF fixture through an incremental update', async () => {
    const template = new Uint8Array(await readFile(new URL('../../renderer-pdf/tests/fixtures/pet-addendum-2.pdf', import.meta.url)))
    const form = {
  ...
  it('matches the Paradoc renderer adapter data shape', async () => {
    ...
    const expected = await existingPdfRenderer().render(request as never)
    expect((await PDFDocument.load(actual)).getForm().getTextField('pet_name').getText()).toBe('Pixel')
    expect((await PDFDocument.load(expected)).getForm().getTextField('pet_name').getText()).toBe('Pixel')
  })
})
