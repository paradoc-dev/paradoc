...
| `not_matching` | A binding key has no PDF field (the message names them), or the layer has no bindings. |
...
| `unknown_bindings_source` | The layer's `bindingsFrom` names no layer. |
