...
`paradoc validate` checks this before any data exists. For each binding to a PDF text field it lays out values at the bound field's length limit (`maxLength` or the `pattern` length). An error: a typical value of that length (the repeated text `The quick brown fox jumps over the lazy dog `) cannot fit at 6 points; an enum's option value cannot fit; a value or split part exceeds a comb field's boxes. A warning: only the widest value (all `W` in Helvetica) overflows, or a bound text field has no `maxLength` or `pattern`. Messages name the layer, PDF field, artifact path, and limit. ALWAYS give every text field bound to a PDF box a `maxLength` the box can hold; measure the box with `paradoc inspect` and size the limit to it, or use a multiline field for free text.
